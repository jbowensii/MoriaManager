...
def _enable_logging(tmp_path, monkeypatch):
    """Patch GamePaths and create a settings.ini that enables logging."""
    monkeypatch.setattr(GamePaths, "CONFIG_DIR", tmp_path)
    monkeypatch.setattr(GamePaths, "SETTINGS_INI_FILE", tmp_path / "settings.ini")
...
class TestSetupLogging:
    ...
    def test_disabled_logging_uses_null_handler(self, tmp_path, monkeypatch):
        monkeypatch.setattr(GamePaths, "CONFIG_DIR", tmp_path)
        monkeypatch.setattr(GamePaths, "SETTINGS_INI_FILE", tmp_path / "settings.ini")
        ini = tmp_path / "settings.ini"
        ini.write_text("[General]\nlogging = false\n", encoding="utf-8")
